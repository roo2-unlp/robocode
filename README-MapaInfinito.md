...
Para lograr esto, se implementó el patrón Strategy, por lo que se creó la interfaz WallCollisionStrategy.java y las clases que la implementan: InfiniteMapWallCollisionStrategy.java y ClassicWallCollisionStrategy.java y se modificó la clase RobotPeer.java para que se instanciara con una estrategia. Dentro de la clase RobotPeer.java hay un método checkWallCollision() que es donde se implementaron los cambios acorde a la estrategia con la que se instanció, teniendo en cuenta que Y es la altura y X es el ancho del mapa:
...
**Clases del código implementadas**

- InfiniteMapWallCollisionStrategy.java - Estrategia que implementa el modo mapa infinito.
- ClassicWallCollisionStrategy.java - Estrategia que implementan el modo mapa clasico.
- WallCollisionStrategy.java - Interfaz de estrategias
- TestInfiniteMapBasicWrap.java - Tests de integración basicos de wrapping en mapas infinitos
- TestInfiniteMapConssitency.java - Tests de integración de consistencia en mapas infinitos
